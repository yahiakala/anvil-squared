...
class MultiSelectChips(MultiSelectChipsTemplate):
    ...
    def update_chips(self, **event_args):
        ...
        for prompt in self._selected:
            self.fp_chips_left.add_component(Chip(item={'name': prompt['key'], 'description': prompt['description']}))
    ...
    def dd_prompts_change(self, **event_args):
        ...
        self.selected = self.selected + [i for i in self.items if i['value'] == selected_item]
    ...
    def apply_filters(self, **event_args):
        ...
        # print(self.filters)
        self.filtered_data = [
            i for i in self._items if i not in self._selected
        ]
        # print(self.filtered_data[:2])

        if self._filters['profession']:
            self.filtered_data = [
                i for i in self.filtered_data if i['categories']["profession"] in self._filters['profession']
            ]
            
        if self._filters['author']:
            self.filtered_data = [
                i for i in self.filtered_data if i['categories']["author"] in self._filters['author']
            ]
            
        if self._filters['language']:
            print_timestamp('filtering language')
            self.filtered_data = [
                i for i in self.filtered_data if i['categories']["language"] in self._filters['language']
            ]
            
        if self._filters['note_type']:
            self.filtered_data = [
                i for i in self.filtered_data if i['categories']["note_type"] in self._filters['note_type']
            ]
