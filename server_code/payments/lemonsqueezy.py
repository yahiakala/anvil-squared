...
def create_checkout(
    ...
    data = {
        "data": {
            ...
            "attributes": {
                ...
                "checkout_options": {"button_color": "#2DD272"},
                "checkout_data": {
            ...
            "relationships": {
                ...
                "variant": {"data": {"type": "variants", "id": str(selected_variant)}},
            },
        }
    }
    ...
    try:
        response = anvil.http.request(
            url=url, method="POST", data=data, json=True, headers=headers
        )
...
def get_customer(api_key=None, customer_id=None):
    ...
    response = anvil.http.request(
        url=cust_url, method="GET", json=True, headers=headers
    )
...
def get_stores(api_key=None, all_info=False):
    ...
    response = anvil.http.request(
        url=store_url, method="GET", json=True, headers=headers
    )
    ...
    if all_info:
        ...
        for store in response["data"]:
            ...
            for product in resp_products["data"]:
                ...
                data[store_counter]["products"][product_counter]["variants"] = [
                    variant for variant in resp_variants["data"]
                ]
                product_counter += 1
...
def get_products(api_key=None, store_id=None):
    ...
    response = anvil.http.request(url=url, method="GET", json=True, headers=headers)
    if "next" in response["links"].keys():
